...
extern "C"
{
  ...
  void ADDON_ReadSettings(void)
  {
     ...
     ***********************************************/
    /* Read setting "usedemuxer" from settings.xml */
     // demcris
    if (!XBMC->GetSetting("usedemuxer", &g_bDemuxing))
    {
      XBMC->Log(ADDON::LOG_ERROR, "Couldn't get 'demuxing' setting. Default: %b", DEFAULT_HANDLE_DEMUXING);
    ...
    {
      XBMC->Log(ADDON::LOG_ERROR, "Couldn't get extra debug setting. Default: %b", DEFAULT_EXTRA_DEBUG);
  ...
  /* TODO: implement real backend version */
  const char *GetBackendVersion(void)
  {
    static const char *strBackendVersion = "Unknown Backend Version";
  ...
  bool SeekTime(int time, bool backwards, double *startpts)
  {
    if (E2STBData != NULL)
    {
      return E2STBData->SeekTime(time, backwards, startpts);
    }
    return false;
  }
  ...
  PVR_ERROR GetStreamProperties(PVR_STREAM_PROPERTIES* pProperties)
  {
    ...
    return E2STBData->GetStreamProperties(pProperties);
  }
...
//  DemuxPacket* DemuxRead(void) { return NULL; }
  ...
  // demcris not used by janbar's demuxer code

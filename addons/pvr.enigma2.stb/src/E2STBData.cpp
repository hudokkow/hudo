...
CE2STBData::CE2STBData()
{
  ...
  m_strServerName = "PVR Enigma2 STB";
...
PVR_ERROR CE2STBData::GetChannels(ADDON_HANDLE handle, bool bRadio)
{
      ...
      iChannelPtr++)
  {
    ...
    if (channel.bRadio == bRadio)
    {
      ...
      if (!g_bUseTimeshift)
      {
        std::string strStream = "pvr://stream/tv/" + E2STBUtils->IntToString(channel.iUniqueId) + ".ts";
...
/**************************************************************************//**
 * Demuxer // Demuxer // Demuxer // Demuxer // Demuxer // Demuxer // Demuxer
 *****************************************************************************/
// demcris
bool CE2STBData::SeekTime(int time, bool backwards, double* startpts)
{
  return m_demux ? m_demux->SeekTime(time, backwards, startpts) : false;
}

void CE2STBData::DemuxAbort(void)
{
  if (m_demux)
  {
    m_demux->Abort();
...
void CE2STBData::DemuxFlush(void)
{
  if (m_demux)
  {
    m_demux->Flush();
...
PVR_ERROR CE2STBData::GetStreamProperties(PVR_STREAM_PROPERTIES* pProperties)
{
  return m_demux && m_demux->GetStreamProperties(pProperties) ? PVR_ERROR_NO_ERROR : PVR_ERROR_SERVER_ERROR;
}

DemuxPacket* CE2STBData::DemuxRead(void)
{
...
PVR_ERROR CE2STBData::AddTimer(const PVR_TIMER &timer)
{
  ...
  std::string strTemp = "web/timeradd?sRef="
      ...
      + E2STBUtils->IntToString(timer.iWeekdays) + "&begin="
      + E2STBUtils->IntToString(timer.startTime) + "&end="
      + E2STBUtils->IntToString(timer.endTime) + "&name="
      + E2STBUtils->URLEncodeInline(timer.strTitle) + "&description="
      + E2STBUtils->URLEncodeInline(timer.strSummary) + "&eit="
      + E2STBUtils->IntToString(timer.iEpgUid);
...
PVR_ERROR CE2STBData::DeleteTimer(const PVR_TIMER &timer)
{
  ...
  std::string strTemp = "web/timerdelete?sRef="
      ...
      + E2STBUtils->IntToString(timer.startTime) + "&end="
      + E2STBUtils->IntToString(timer.endTime);
...
PVR_ERROR CE2STBData::UpdateTimer(const PVR_TIMER &timer)
{
  ...
  std::string strTemp = "web/timerchange?sRef="
      ...
      + E2STBUtils->IntToString(iDisabled) + "&justplay=0&repeated="
      + E2STBUtils->IntToString(timer.iWeekdays) + "&channelOld="
      + E2STBUtils->URLEncodeInline(strOldServiceReference) + "&beginOld="
      + E2STBUtils->IntToString(oldTimer.startTime) + "&endOld="
      + E2STBUtils->IntToString(oldTimer.endTime) + "&deleteOldOnSave=1";
